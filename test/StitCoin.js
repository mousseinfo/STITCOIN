...
contract('StitCoin', function(accounts){
    ...
    it('initializes the contract with the correct values', function(){
        return StitCoin.deployed().then(function(instance){
            ...
            assert.equal(name, 'StitCoin', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol, 'STIT', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'StitCoin v1.0', 'has the correct standard');
        });
    })
    ...
    it('transfers token ownership', function(){
        return StitCoin.deployed().then(function(instance){
            ...
            assert(error.message.includes('revert') >= 0, 'error message must contain revert');
